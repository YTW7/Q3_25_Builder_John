...
describe("capstone_launchpad", () => {
  ...
  console.log("initialVaultX:", initialVaultY.amount.toString());
...
it("Second deposit (must be proportional)", async () => {
  const amount = new anchor.BN(100_000_000); // wants 100 LP tokens
  const maxX = new anchor.BN(600_000_000);   // provides enough slack
  const maxY = new anchor.BN(400_000_000);
  ...
  console.log("Initial balances (before 2nd deposit):");
  console.log("User X:", initialUserX.amount.toString());
  console.log("User Y:", initialUserY.amount.toString());
  console.log("User LP:", initialUserLp.amount.toString());
  ...
  console.log("Final balances (after 2nd deposit):");
  console.log("User X:", finalUserX.amount.toString());
  console.log("User Y:", finalUserY.amount.toString());
  console.log("User LP:", finalUserLp.amount.toString());
  console.log("Vault X:", finalVaultX.amount.toString());
...
// Fetch LP token supply (raw, unadjusted for decimals)
const lpSupplyInfo = await connection.getTokenSupply(lpMintPda);
const lpSupply = BigInt(lpSupplyInfo.value.amount);

// Expected proportional amounts
const requiredX =
  (BigInt(amount.toString()) * initialVaultX.amount) / lpSupply;

const requiredY =
  (BigInt(amount.toString()) * initialVaultY.amount) / lpSupply;

  // Assertions
  expect(finalUserX.amount).to.equal(initialUserX.amount - requiredX);
  expect(finalUserY.amount).to.equal(initialUserY.amount - requiredY);
  expect(finalUserLp.amount).to.equal(initialUserLp.amount + BigInt(amount.toString()));
  expect(finalVaultX.amount).to.equal(initialVaultX.amount + requiredX);
  expect(finalVaultY.amount).to.equal(initialVaultY.amount + requiredY);
});
